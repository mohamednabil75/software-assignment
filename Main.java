...
class Main {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            float priceDiscount;
            if (menu instanceof VegMenuEastern || menu instanceof NonVegMenuEastern
                    || menu instanceof KidsMenuEastern) {
                discountmethod = new DiscountMethod(new EasternPizzaDiscount());
            }
            if (menu instanceof VegMenuItalian || menu instanceof NonVegMenuItalian
                    || menu instanceof KidsMenuItalian) {
        ...
        String lineOrder = "Recived an order ";
        float totalcost = 0;
        ...
        // end of payment
        lineOrder = "Order has been prepared";
        orderObs.removeObs(waiter);
        orderObs.setState(lineOrder);
        orderObs.removeObs(kitchen);
        lineOrder="Has took the order";
        orderObs.addObs(waiter);
        orderObs.setState(lineOrder);
        orderObs.removeObs(waiter);
        scan.close();
